...
class ProximalPairs():
    ...
    def num_trial_pairs(self):  #Reindexed with geneList
        genecount = dd.DataFrame.from_dict(Counter(self.curr_cell_df.index) , orient='index').reindex(self.geneList).fillna(0)
    ...
    def obs_trial_pairs(self): #debug for large d
        ...
        pairs = Counter(pairs)
        pairs = dd.Series(pairs).reset_index()
        if len(pairs):
            obs_df = dd.pivot_table(pairs, index = 'level_0', columns ='level_1', values = 0).fillna(0)
            ...
            obs_df = dd.DataFrame(0, self.geneList, self.geneList)
...
class Instant():
    ...
    def load_preprocessed_data(self, data):
        self.df = dd.read_csv(data).set_index('gene')
    ...
    def preprocess_and_load_data(self, expression_data, barcode_data):
        ...
        df = dd.read_csv(self.expression_data)
    ...
    def load_codebook(self):
        codebook = dd.read_csv(self.barcode_data, converters={'bit_barcode': lambda x: int(str(x)[::-1],2)})
    ...
    def _calculate_ProximalPairs(self, args):
        ...
        pp_model = ProximalPairs(geneList = self.geneList, df_loc = self.df[self.df.uID == cell_id][['absX', 'absY']], distance_threshold = self.distance_threshold)
    ...
    def run_ProximalPairs(self, distance_threshold, min_genecount, pval_matrix_name = None, gene_count_name = None):
        ...
        for i, cell_id in enumerate(cell_ids):
           ...
           if self.df[self.df.uID == cell_id].shape[0] > min_genecount:
               #valid_cell_data.append([i, df_id])
               valid_cell_data.append([i, cell_id])
               #valid_cell_data.append([i, self.df[self.df.uID == cell_id][['absX', 'absY']]])
           else:
               print(f"min genecount less than {min_genecount} for cell id {cell_id}, Skipping ...")
        #    if len(valid_cell_data) % 8 == 0 and len(valid_cell_data) > 0:
        #        print(f"Running PP now on {self.threads} threads, iter {i}, {len(valid_cell_data)}")
        #        pool = mp.Pool(self.threads)
        ...
        #        valid_cell_data = []
        #        print("TIme for 1 PP: ", timeit.default_timer() - check)
    ...
    def _unstack_df_both(self, obs, present_cells, alpha, second_df_name = 'Expected coloc'):
        ...
        pairwise_p_val_df = dd.DataFrame(data)
    ...
    def run_GlobalColocalization(self, alpha_cellwise = 0.01, min_transcript = 0, show_det_pairs = 0, high_precision = False, glob_coloc_name = None, exp_coloc_name = None, unstacked_pvals_name = None):
        ...
        self.global_coloc_df = dd.DataFrame(global_coloc, index = self.geneList, columns = self.geneList)
        self.expected_coloc_df = dd.DataFrame(expected_coloc, index = self.geneList, columns = self.geneList)
        if glob_coloc_name:
            self._save_globcolocal_csv(glob_coloc_name)
        if exp_coloc_name:
            self._save_expcolocal_csv(exp_coloc_name)
        #self.global_coloc_df = dd.read_csv(glob_coloc_name, index_col=0)
        #self.expected_coloc_df = dd.read_csv(exp_coloc_name, index_col=0)
